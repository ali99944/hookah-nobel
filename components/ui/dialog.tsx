      ...
      {isOpen && (
        <div className="fixed inset-0 z-2000 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className={`relative w-full ${width} bg-white border border-border shadow-none rounded-3xl flex flex-col`}
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-4 border-b border-neutral-100">
              ...
              <button 
                onClick={onClose}
                className="cursor-pointer p-1 text-neutral-400 hover:text-neutral-900 hover:bg-neutral-100 transition-colors rounded-3xl"
              >
                <X size={20} />
              </button>
            </div>
...
export function DangerDialog({ isOpen, onClose, onConfirm,confirmText, cancelText, title, description, isLoading }: ActionDialogProps) {
  return (
    <Dialog isOpen={isOpen} onClose={onClose} title={title}>
      <div className="flex flex-col gap-4">
        <div className="flex gap-4">
